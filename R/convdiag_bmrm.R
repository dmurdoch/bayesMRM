...
convdiag_bmrm <- function(x , var="P", convdiag="geweke",print=TRUE,...){
  ...
  if (!inherits(x,'bmrm')) { 
    stop("incorrect class of 'x.bmrm' ",call.=FALSE)}
  T<-x$nobs
  q<-x$nsource
  J<-x$nvar
  ...
  #a Markov chain (by default the first 10% and the last 50%)
    ...
    if(inherits(geweke1,"list")) geweke1=geweke1[[1]]
  ...
  if ("heidel" %in% convdiag){
    ...
    if(inherits(heidel1,"list")) heidel1=heidel1[[1]]
  ...
  if ("raftery" %in% convdiag){
    ...
    if(inherits(raftery1,"list")) raftery1 =raftery1 [[1]]
